$(document).ready(function () {
    ...
    var onCompletion = (res) => {
        ...
        if(res.class==1){
            ...
            result=`Verilen özelliklere göre, sizin ${(prob*100).toFixed(2)}% meme kanseri geliştirme olasılığı`
        }
        else{
            $('#result').removeClass('text-danger').addClass('text-success');
            result=`Verilen özelliklere göre meme kanserine yakalanma olasılığınız düşüktür`
        }
        $('#result').html(result);
        $('#result-row').show();
        $('#result-heading').show();
